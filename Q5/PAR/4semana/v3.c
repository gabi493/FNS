...

{
    ...
    for (k=0; k<N; k++) {
      ...
      char task_name[30] = "transpose_xy_planes_";
...

{
    ...
    for (k=0; k<N; k++) {
      ...
      char task_name[30] = "transpose_zx_planes_";
...

{
    ...
    for (k=0; k<N; k++)  {
      ...
      char task_name[15] = "ffts1_planes_";
...

{
  ...
  double stamp;
    ...
    //tareador_start_task("ffts1_planes_in");

    ffts1_planes(p1d, in_fftw);

    //tareador_end_task("ffts1_planes_in");



    //tareador_start_task("transpose_xy_planes_in");

    transpose_xy_planes(tmp_fftw, in_fftw);

    //tareador_end_task("transpose_xy_planes_in");



    //tareador_start_task("ffts1_planes_tmp");

    ffts1_planes(p1d, tmp_fftw);

    //tareador_end_task("ffts1_planes_tmp");



    //tareador_start_task("transpose_zx_planes_tmp");

    transpose_zx_planes(in_fftw, tmp_fftw);

    //tareador_end_task("transpose_zx_planes_tmp");



    //tareador_start_task("ffts1_planes_in_bis");

    ffts1_planes(p1d, in_fftw);

    //tareador_end_task("ffts1_planes_in_bis");



    //tareador_start_task("transpose_zx_planes_in");

    transpose_zx_planes(tmp_fftw, in_fftw);

    //tareador_end_task("transpose_zx_planes_in");



    //tareador_start_task("transpose_xy_planes_tmp");

    transpose_xy_planes(in_fftw, tmp_fftw);

    //tareador_end_task("transpose_xy_planes_tmp");
    ...
    for (k = 0; k < N; k++)
      ...
      for (j = 0; j < N; j++)
...

         {
           ...
           if (!((fabs(in_fftw[k][j][i][0]-out_fftw[k][j][i][0]) < ERROR_ALLOWED) && (fabs(in_fftw[k][j][i][1]-out_fftw[k][j][i][1]) < ERROR

_ALLOWED))) {

              printf ("\n\n\nError: %d %d %d,   %f %f %f %f errors so far %d\n",k,j,i, in_fftw[k][j][i][0],out_fftw[k][j][i][0],in_fftw[k][j

][i][1],out_fftw[k][j][i][1],++errors_count);
